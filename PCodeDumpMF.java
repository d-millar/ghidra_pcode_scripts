...
public class PCodeDump extends GhidraScript {
	...
	protected void run() throws Exception {
		...
		try {
			File f = new File(outputDirectory,"input_assist.dl");
			ia = new PrintWriter(f);
			pws.put("input_assist.dl", ia);
			ia.println(".decl S2N(key:symbol, n:number)");
			ia.println(".input S2N");
	...
	private void export(String label, String key, String value) {
		try {
			String fname = label+".facts";
			PrintWriter pw = pws.get(fname);
			if (pw == null) {
				File f = new File(outputDirectory, fname);
				pw = new PrintWriter(f);
				pws.put(fname, pw);
				ia.println(".decl "+label+"(key:symbol, value:symbol)");
				ia.println(".input "+label);
			}
			if (!pairs.containsKey(label+key)) {
				pw.println(key+TAB+value);
	...
	private void exportL(String label, String key, long value) {
		try {
			...
			if (pw == null) {
				...
				pw = new PrintWriter(f);
				pws.put(fname, pw);
				ia.println(".decl "+label+"(key:symbol, value:number)");
				ia.println(".input "+label);
			}
			if (!pairs.containsKey(label+key)) {
				pw.println(key+TAB+value);
	...
	private void exportN(String label, String key, int index, String value) {
	        if (value == null) {
			System.err.println(label+" key:"+key+" index:"+index+" NULL");
			return;
	        }
		try {
			String fname = label+".facts";
			...
			if (pw == null) {
				...
				pws.put(fname, pw);
				if (value.equals("")) {
					ia.println(".decl "+label+"(key:symbol, n:number)");
				} else {
					ia.println(".decl "+label+"(key:symbol, n:number, value:symbol)");
				}
				ia.println(".input "+label);
			}
			if (!pairs.containsKey(label+key+index)) {
				pw.println(key+TAB+index+TAB+value);
				pairs.put(label+key+index, value);
			}
			//System.out.println(label+"\t"+key+"\t"+index+"\t"+value);
			exportS2N(index);
		} catch (FileNotFoundException e) {
			e.printStackTrace();
		}		
	...
	private void exportNL(String label, String key, int index, long value) {
		try {
			...
			if (pw == null) {
				...
				pw = new PrintWriter(f);
				pws.put(fname, pw);
				ia.println(".decl " + label + "(key:symbol, n:number, value:number)");
				ia.println(".input " + label);
			}
			if (!pairs.containsKey(label + key + index)) {
				pw.println(key + TAB + index + TAB + value);
	...
	private void exportS2N(int n) {
		if (!pairs.containsKey("S2N"+n)) {
			pws.get("S2N").println(Integer.toHexString(n)+TAB+n);
			pairs.put("S2N"+n, Integer.toHexString(n));
		}
	}
